...
// COMANDO PARA COMPILAR: gcc -o Questao02 Questao02.c -pthread
...
typedef struct {
    char name;
    int burst_time, position, priority;
} Process;
...
// Gera um valor de prioridade aleatório no intervalo [0, LIMIT)
int generate_priority () {
    return rand() % LIMIT;
}

// Imprime informações relevantes sobre os processos (nome, prioridade e tempo de execução restante)
...
void choose_next () {
    ...
    int next_position, highest_priority = INT_MIN;
    ...
    for (int i = 0; i < N; i++) {
        ...
        // Verifica se o processo ainda não terminou de ser executado e se tem a maior prioridade
    ...
    // Se nem todos os processos terminaram, a vez será do processo com maior prioridade
    if (!finished_all) {
        turn = next_position;
    }
...
void *run (void *tid) {
    ...
    while (processes[i].burst_time > 0) {
        ...
        // Atualiza as prioridades, aumentando a prioridade dos processos que não foram executados e diminuindo a do processo que acabou de ser executado
        for (int i = 0; i < N; i++) {
            if (processes[i].burst_time > 0) {
                if (i == turn) {
                    processes[i].priority = max(0, processes[i].priority - 1);
                }
